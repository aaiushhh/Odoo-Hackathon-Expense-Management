...
exports.getPendingExpenses = async (req, res) => {
  try {
    ...
    const pendingForUser = approvalFlows.filter(flow => {
      ...
      const userInSequence = flow.sequence.some(approver => 
        approver._id.toString() === user._id.toString()
      );
      
      if (!userInSequence) return false;

      // Check if user has already approved
      const userApproved = flow.approvals.some(approval => 
        approval.approverId._id.toString() === user._id.toString()
      );
...
exports.makeApprovalDecision = async (req, res) => {
  try {
    ...
    const userInSequence = approvalFlow.sequence.some(approver => 
      approver._id.toString() === user._id.toString()
    );
    ...
    // Check if user has already approved
    const userApproved = approvalFlow.approvals.some(approval => 
      approval.approverId.toString() === user._id.toString()
    );
    ...
    if (decision === 'REJECTED') {
      ...
      const approvalsCount = approvalFlow.approvals.filter(a => a.decision === 'APPROVED').length;
